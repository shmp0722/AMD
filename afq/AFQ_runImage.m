function [afq patient_data control_data norms abn abnTracts] = AFQ_runImage(sub_dirs, sub_group, afq)
...
%         afq = AFQ_set(afq,'tract profile','subnum',ii,TractProfile);
        ...
        if numimages > 0;
            for jj = 1:numimages
                ...
                %
                fg_group = fieldnames(afq.files.fibers);
                
                if length(fg_group)>4
                    for   ll = 5:length(fg_group)
                        fgfile = fgRead(afq.files.fibers.(fg_group{ll}){ii});
                        
                        % Compute a Tract Profile for that image
                        imagevals = AFQ_ComputeTractPropertiesImage(fgfile, image, afq.params.numberOfNodes, afq.params.clip2rois, sub_dirs{ii}, fWeight, afq);
                        % Add values to the afq structure
                        afq.vals.(afq.files.images(jj).name){20+ll-4}(ii,:)=imagevals;
