...
% nnodes =  size(afq.patient_data(1).FA);
nnodes = [6,45];
...
%     set(A4,'FaceColor',[1 1 1],'linestyle','none')
    ...
    m  = nanmean(pt,1);
    plot(1:length(pt(k,:)),m,'Color',[1  0  0] ,'linewidth',3)
    
    ylabel(upper(property))
    xlabel('Location')
    
    % add stats
    for jj = 1:length(pt(k,:))
...
%% save figure
pvalue = num2str(Alpha);
saveas(gca, sprintf('RerevisionForBSF/AFQ_Merged_nn50_%s%s.pdf',...
    property,pvalue(3:4)))
