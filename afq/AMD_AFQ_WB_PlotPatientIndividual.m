...
for v = 1:length(valname)
    ...
    % make fgnames shorter
    newfgNames = AFQ_get(afq,'fgNames');
...
FGN = fieldnames(afq.files.fibers);

% fg_p = AFQ_get(afq,'clean fg',s);

% First we render the first fiber tract in a new figure window
% fg = fgRead(afq.files.fibers.(FGN{5}){s});
% lightH = AFQ_RenderFibers(fg,'color',colors(1,:),'numfibers',50,'newfig',0);

colors = jet(8);

for  kk = 3:10
    fg = fgRead(afq.files.fibers.(FGN{kk*2}){1});
    AFQ_RenderFibers(fg,'color',colors(kk-2,:),'numfibers',100,'newfig',0);
end

% % Make a CC roi
% roiDir = '/media/USB_HDD1/dMRI_data/AO-1-YS-20150531/ROIs';
% rois = dir(fullfile(roiDir,'*_CC_*mat'));
%
% roi = dtiNewRoi();
% for jj = 1:5
%     cur_ROI = dtiReadRoi(fullfile('/media/USB_HDD1/dMRI_data/AO-1-YS-20150531/ROIs',rois(jj).name))
%
...
for ll =3;% 2:28
    ...
    for  kk = 3:10
        fg = fgRead(afq.files.fibers.(FGN{kk*2}){ll}); % sub_dirs{26} = YM
        AFQ_RenderFibers(fg,'color',colors(kk-2,:),'numfibers',100,'newfig',0);
    end
...
for jj = 1:nfg
    ...
    [h(jj,:),p(jj,:),~,Tstats(jj)] = zscore(afq.patient_data(jj).FA,afqC.control_data(jj).FA);
end

%% Render figure showing OT OR
% Load ACH

load /media/USB_HDD1/dMRI_data/Results/ACH_0402.mat

%% load OT, OR

otDir = '/media/USB_HDD1/dMRI_data/AO-1-YS-20150531/dwi_1st/fibers/conTrack/OT_5K';
orDir = '/media/USB_HDD1/dMRI_data/AO-1-YS-20150531/dwi_1st/fibers/conTrack/OR_100K';
...
otDir = '/media/USB_HDD1/dMRI_data/Ctl-13-MW-20140313-dMRI-Anatomy/dwi_1st/fibers/conTrack/OT_5K';
orDir = '/media/USB_HDD1/dMRI_data/Ctl-13-MW-20140313-dMRI-Anatomy/dwi_1st/fibers/conTrack/OR_100K';
...
for  kk = 3:10
    fg = fgRead(afq.files.fibers.(FGN{kk*2}){s});
