function AMD_plotDiffuisonMeasure(vals,fibID,SavePath)
...
%     pac(1:8,2)= val_AMD(:,jj);
%     
%     [p(jj),~,stats(jj)] = signrank(pac,[],'off');
% %     co = multcompare(stats(jj),'display','off');
% %     C{jj}=co;
% end
% Portion =  p<0.05; % where is most effected
...
% % add individual val plot
% [M,nodes] = size(CTL_data);
% for k = 1:M %1:length(subDir)
%     plot(X,CTL_data(k,:),'--','Color',[1 1 1],...
%         'linewidth',1);
% end
...
Mean   = nanmean(AMD_data,1);
plot(X,Mean,'Color',c(3,:) ,'linewidth',3)
...
for jj= 1: nodes
    ...
    p(jj) = signrank(pac(:,2),nanmean(pac(:,1)));
%     [p(jj),~,stats(jj)] = signrank(pac(:,2),mean(pac(:,1)));
